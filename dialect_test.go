...
var allDialectSyntax = []dialectSyntax{
	dialectSyntax{
		...
		`SELECT "name", "grade", "score" FROM "student" WHERE (grade > $1) AND ((score <= $2) OR (score >= $3)) ORDER BY "name" DESC LIMIT $4 OFFSET $5`,
...
func DoTestCreateTableSql(assert *assrt.Assert, info dialectSyntax) {
	...
	model := structPtrToModel(table, true)
	...
	model = structPtrToModel(table2, true)
...
func DoTestInsertSQL(assert *assrt.Assert, info dialectSyntax) {
	assert.Logf("Dialect %T\n", info.dialect)
	model := structPtrToModel(sqlGenSampleData, true)
...
func DoTestSelectionSQL(assert *assrt.Assert, info dialectSyntax) {
	...
	model := structPtrToModel(new(Post), true)
...
func DoTestQuerySQL(assert *assrt.Assert, info dialectSyntax) {
	...
	model := structPtrToModel(new(Student), true)
	criteria := new(Criteria)
	criteria.model = model
	condition := NewCondition("grade > ?", 6)
